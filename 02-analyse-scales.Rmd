...
library(tidyverse)
require(cocor) # used for correlation tests later on
...
## H2: The scores are consistent with those reported by Tobia (2020)

```{r}
h2 <- scores %>% 
  select(individual, ends_with("est")) %>% 
  rename_with(~ str_remove(., "_est")) %>% 
  left_join(tobia_percentages %>% select(individual = item_num, tobia = tobia_percentage), by = "individual") %>% 
  select(-individual) %>% 
  as.matrix()

round(cor(h2, method = "spearman"),3)
```

Test the difference between correlations

```{r}
cocor(~tobia + vehicle | tobia + violation, data = h2, test = "hittner2003")
#sig different p-value = 0.0389
```

```{r}
cocor(~tobia + nuisance | tobia + violation, data = h2, test = "hittner2003")
#no sig different p-value = 0.3206
```

```{r}
cocor(~tobia + vehicle | tobia + nuisance, data = h2, test = "hittner2003")
#no sig difference p-value = 0.0369
```



## H3: The scores are consistent across the Vehicle, Violation and Nuisance conditions

```{r}
h3 <- cbind(nuisance = scores$nuisance_est, vehicle  = scores$vehicle_est, violation = scores$violation_est)
...
```{r}
cocor(~nuisance + vehicle | nuisance + violation, data = h3, test = "hittner2003")
